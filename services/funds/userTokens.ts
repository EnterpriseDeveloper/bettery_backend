...
const balanceCheck = async (wallet: string) => {
  let balance: any = await axios
    .get(`${demonAPI}:1317/cosmos/bank/v1beta1/balances/${wallet}`)
    .catch((err) => {
      return {
        status: 400,
  ...
  if (balance.status == 400) {
    ...
    bet = balances.find((x: any) => {
      return x.denom == "bet";
    });
    if (bet) {
      bet = bet.amount;
    }
    bty = balances.find((x: any) => {
      return x.denom == "bty";
    });
    if (bty) {
      bty = bty.amount;
    ...
    let betToken = bet
      ? Number(Number(web3.utils.fromWei(bet, "ether")).toFixed(2))
      : 0;
    let btyToken = bty
      ? Number(Number(web3.utils.fromWei(bty, "ether")).toFixed(2))
      : 0;
