...
const payToRefferers = async (id: any) => {
  ...
  let { payRefAddr, payRefAmount, payComp } = getRefStruct(
    allData,
    "none",
    refAmount,
    mintedTokens,
  ...
  const msg = {
    typeUrl: "/bettery.funds.v1.MsgCreateMintBet",
    value: {
      creator: address,
      pubId: id,
...
const fetchDataFromDb = async (id: any) => {
  ...
  return await axios.post(`${path}/query`, fetchData).catch((err) => {
    console.log(
      "DB error from query payToRefferers: " + err.response.data.message,
    );
    return;
  });
};
...
const letFindAllRef = (players: any) => {
  ...
  for (let i = 0; i < players.length; i++) {
    ...
    if (players[i]["publicActivites/from"]["users/invitedBy"]) {
      let walletRef =
        players[i]["publicActivites/from"]["users/invitedBy"]["users/wallet"];
      let findRef = allData.findIndex((x) => {
        return x.wallet == walletRef && x.level == 0;
      });
      if (findRef == -1) {
        allData.push({
          wallet: walletRef,
      ...
      //  find L2
      if (
        players[i]["publicActivites/from"]["users/invitedBy"]["users/invitedBy"]
      ) {
        let walletRef =
          players[i]["publicActivites/from"]["users/invitedBy"][
            "users/invitedBy"
          ]["users/wallet"];
        let findRef = allData.findIndex((x) => {
          return x.wallet == walletRef && x.level == 1;
        });
        if (findRef == -1) {
          allData.push({
            wallet: walletRef,
        ...
        if (
          ...
          let walletRef =
            players[i]["publicActivites/from"]["users/invitedBy"][
              "users/invitedBy"
            ]["users/invitedBy"]["users/wallet"];
          let findRef = allData.findIndex((x) => {
            return x.wallet == walletRef && x.level == 2;
          });
          if (findRef == -1) {
            allData.push({
              wallet: walletRef,
...
const calcTokens = (
  ...
  return allData.map((x: any) => {
    return {
      ...x,
      tokens:
        (((percent[x.level] * mintedTokens) / 100) * x.amount) /
        refAmount[x.level],
    };
  });
};
  ...
  if (refAmount[0] > 0) {
    for (let i = 0; i < allData.length; i++) {
      if (allData[i].level == 0) {
        ...
        payRefAmount[0].push(
          web3.utils.toWei(String(allData[i].tokens), "ether"),
        );
      }
    }
    if (refAmount[1] > 0) {
      ...
      if (refAmount[2] > 100) {
        ...
        payRefAmount[2][0] = "0";
        payComp = web3.utils.toWei(
          String((contrPercet[2] * mintedTokens) / 100),
          "ether",
        );
      }
    } else {
      ...
      payRefAmount[2][0] = "0";
      payComp = web3.utils.toWei(
        String(((contrPercet[1] + contrPercet[2]) * mintedTokens) / 100),
        "ether",
      );
    }
  } else {
    ...
    payComp = web3.utils.toWei(
      String(
        ((contrPercet[0] + contrPercet[1] + contrPercet[2]) * mintedTokens) /
          100,
      ),
      "ether",
    );
...
let getDataUser = async (id: number, finalAnswerNumber: number) => {
  ...
  let dataFromDB: any = await axios.post(`${path}/query`, data).catch((err) => {
    console.log(
      "DB error getDataUser from  payToRefferers: " + err.response.data.message,
    );
    return;
  });

  return dataFromDB.data[0]["publicEvents/parcipiantsAnswer"].filter(
    (el: any) => {
...
let sendToDbPayRef = (
  ...
  let data = allWinners
    .map((el: any) => {
      ...
      if (el["publicActivites/from"].invitedBy) {
        index = payRefAddr.indexOf(
          el["publicActivites/from"].invitedBy["users/wallet"],
        );
  ...
  if (data.length) {
    axios.post(`${path}/transact`, data).catch((err) => {
      console.log(
        "DB error from send to DB refIndo: " + err.response.data.message,
      );
      return;
    });
  }
